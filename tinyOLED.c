...
void I2C_write(uint8_t data) {
  uint8_t i;
  for(i = 8; i; i--) {            // transmit 8 bits, MSB first
...
void OLED_init(void) {
  ...
  uint8_t i;               // set command mode
  for (i = 0; i < OLED_INIT_LEN; i++) I2C_write(pgm_read_byte(&OLED_INIT_CMD[i])); // send the command bytes
...
void OLED_printC(char ch) {
  ...
  I2C_write(0x00);
  uint8_t i;                        // print spacing between characters
  for(i=5; i; i--) I2C_write(pgm_read_byte(&OLED_FONT[offset++])); // print character
  I2C_stop();
}
...
void OLED_clear(void) {
  ...
  uint16_t i;                             // set data mode
  for(i=512; i; i--) I2C_write(0x00); // clear the screen
  I2C_stop();                             // stop transmission
